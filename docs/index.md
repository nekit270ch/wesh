...
`proc.run <имя процесса>[, <аргументы>][, <читать stdout>][, <verb>]` - запустить процесс с указанным именем файла и аргументами. Если `читать stdout` - `true`, вернуть данные из StdOut процесса. Если `verb` указан, использовать его.
...
`http.server <путь к папке>[, <тихий режим>]` - запустить локальный HTTP-сервер с содержимым указанной папки. Если `тихий режим` указан и равен `true`, не выводить в консоль сообщения о запросах сервера.
### Автоматизация
...
`hotkey.set <клавиша>, <блок кода>` - задать выполнение блока кода при нажатии указанной клавиши.
...
`gui.createForm <заголовок>, <длина>, <ширина>` - создать форму и вернуть ее ID.
