...
`ife [!], <блок кода 1>, <блок кода 2>, <блок кода 3>` - если результат выполнения блока кода 1 - `true`, выполнить блок кода 2, иначе - блок кода 3. Если первым аргументом указан восклицательный знак, то блок кода 2 выполняется, если результат выполнения блока кода 1 равен `false`, а не `true`.

`while <условие>, <блок кода>` - пока условие истинно, выполнять блок кода.

`whilee [!], <команда>, <блок кода>` - пока результат выполнения команды равен `true`, выполнять блок кода. Если первым аргументом указан восклицательный знак, то блок кода выполняется, пока результат выполнения команды равен `false`, а не `true`.
...
`proc.injectDll <id процесса>, <путь к dll>` - внедрить DLL в указанный процесс.
...
`win32.callDll <путь к dll>, <имя или номер функции>, <тип возвращаемого значения>, <аргументы>` - вызвать функцию из DLL и вернуть результат ее выполнения. Синтаксис аналогичен синтакисису [rundll34](https://github.com/nekit270ch/rundll34#%D0%B8%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5).
### Буфер обмена
...
### COM-объекты
`com.create <ProgId>` - создать экземпляр COM-объекта по его ProgId. Возвращает ID объекта.

`com.getProp <id объекта>, <свойство>` - получить значение указанного свойства COM-объекта.

`com.setProp <id объекта>, <свойство>, <значение>` - установить значение указанного свойства COM-объекта.

`com.invokeMethod <id объекта>, <метод>[, <аргументы>]` - вызвать указанный метод COM-объекта.
### .NET-объекты
`net.create <тип>[, <аргументы конструктора>]` - создать экземпляр .NET-объекта по его типу. Возвращает ID объекта.

`net.getProp <id объекта>, <свойство>` - получить значение указанного свойства .NET-объекта.

`net.setProp <id объекта>, <свойство>, <значение>` - установить значение указанного свойства .NET-объекта.

`net.invokeMethod <id объекта>, <метод>[, <аргументы>]` - вызвать указанный метод .NET-объекта.
